/*
TODO: Cooldown Bar
TODO: Build a whole level. just to  adjust the water mechanic
...
public class water : MonoBehaviour
{
    ...
    [Header("Water Settings")]
    [SerializeField] private bool enableWaterStorage = true;
    [SerializeField] private bool enableDynamicForceControl = false;
    ...
    {
        if (enableWaterStorage && currentStorage <= 0) return;

        if (enableDynamicForceControl)
        {
            _initialBoostDistance = Vector2.Distance(hoseTip.position, cursor.position);
            
            _storedBoostMultiplier = Mathf.Lerp(
                _minBoostMultiplier, 
                1f, 
        ...
        {
            _storedBoostMultiplier = 1f; // Default to full force
        }
    ...
    private void Update()
    {
        ...
        if (enableWaterStorage)
        {
            // Existing water storage logic
    ...
    {        
        ...
        float finalBoostForce = enableDynamicForceControl 
            ? boostForce * _storedBoostMultiplier 
            : boostForce;
